@echo off
REM 'dir > null' resets the internal %ERRORLEVEL% to 0. We must do this because some internal cmd like "SET" or "PATH" does not reset "ERRORLEVEL".
dir > nul
...
make%gmu_MAKEEXESUFFIX% gmp_bc_DEBUG=1 %* 2>&1 | tee "%gmu_LOG_OUTPUT_FILENAME%"
goto END

:NoLogOutput
make%gmu_MAKEEXESUFFIX% gmp_bc_DEBUG=1 %*
GOTO END
