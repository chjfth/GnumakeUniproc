...
/*  fprintf(stderr, "Execute simple command %s -- %d\n", command_line, __LINE__); */
         ...
         else
         {
            if ((0 != from_thread) && (NULL == builtin))
            {
               ...
               sprintf(acCommandLine, "%s \"%s\"", pcShellName, temp_name);
               sprintf(acCommandWords, "%s#%s", pcShellName, temp_name);
               ...
               for (warg = words->next; NULL != warg ; warg=warg->next)
               {
                  ...
                  strcat(acCommandLine, " \"");
                  strcat(acCommandLine, warg->word->word);
                  strcat(acCommandLine, "\"");

                  strcat(acCommandWords, "#");
                  strcat(acCommandWords, warg->word->word);
               }               

               wlist = list_string (acCommandWords, "#", 0);
