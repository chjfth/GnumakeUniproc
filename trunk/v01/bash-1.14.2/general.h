...
/*#define JOERG_DBG  */
#ifdef JOERG_DBG
#define JDBG(a,b) { FILE *fdbgOut = NULL; \
      char fName[1024] = ""; sprintf(fName, "bash_%x_%x.dbg", nt_get_process_id(), nt_get_thread_id()); \
      fdbgOut = fopen(fName, "a");                   \
      fprintf(fdbgOut, "%s:%d(%d:%d): " a " \n", __FILE__, __LINE__, nt_get_process_id(), nt_get_thread_id(), b); \
      fclose(fdbgOut); }
#else
#define JDBG(a,b)
#endif
