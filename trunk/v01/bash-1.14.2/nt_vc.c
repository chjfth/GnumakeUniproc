		...
		const char ** export_env)
{
    ...
    char * nt_env = make_nt_env(export_env);
 ...
 /*   fprintf(stderr, "spawnve_console %s -- %s\n", command, exp_args); fflush(stderr); */
...
#define my_spawnve(a,b,c,d) my_spawnve_x(a,b,c,d)
 ...
 */
pid_t
nt_execute_disk_command (words, redirects, command, pipe_in, pipe_out,
   ...
   */
   ...
   /* nt_leave_critsec(__FILE__, __LINE__);*/
   {
      ...
      else
      {
         ...
         pid = my_spawnve(mode,((pcShortCommand && !shell)? pcShortCommand:args[0]), args, spawn_env);
	...
	if (pid == -1) { /* spawn failed */
      ...
      pid = shell_execve_async(command,args,export_env, _P_WAIT != mode);
      goto _quit_;
	}
	 ...
	 SHELL_VAR * func ;
{
   ...
   char * temp_name = 0; /* name of temporary file */
	...
	if (async || pipe_in != NO_PIPE || pipe_out != nt_pipe_filedes[1]) exec_disk = 1 ;
    ...
    if (!exec_disk)
    {
       ...
       if (ret > -1)
       {
          nt_disk_pipe_in = OPEN( lastTempfileName() /* nt_unlink_name */ ,O_RDONLY);
          if (nt_disk_pipe_in < 0) {
             ret = -1 ;
             perror("bash temp file");
          }
          dup2(nt_disk_pipe_in,nt_pipe_filedes[0]);
       }
...
int execve_nt(char *command, char**args, char**env, int asynchronous)
{
   ...
   spawn_env = nt_make_spwan_env(env);
